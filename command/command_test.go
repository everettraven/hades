...
func TestCommand(t *testing.T) {
	...
	for i := 0; i < len(unitTests.UnitTests); i++ {
		...
		defer t.Logf("Removing Docker Image %s", curTest.Image)
		defer curTest.RemoveImage()
		...
		defer t.Logf("Removing Container %s", curTest.ContainerName)
		defer curTest.RemoveContainer()
		defer t.Logf("Stopping Container %s", curTest.ContainerName)
		defer curTest.StopContainer()
		...
		// If it failed any of the command tests lets fail now
		if t.Failed() {
			t.FailNow()
		}
	}
